...
class LocalTCN(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.original_seq_len = input_shape[1]
        if self.original_seq_len % self.n_chunks != 0:
            self.padded_seq_len = ((self.original_seq_len // self.n_chunks) + 1) * self.n_chunks
            total_pad = self.padded_seq_len - self.original_seq_len
            
            self.pad_start = total_pad // 2
            self.pad_end = total_pad - self.pad_start
        else:
            self.padded_seq_len = self.original_seq_len
            self.pad_start = 0
            self.pad_end = 0
            
        self.chunk_size = self.padded_seq_len // self.n_chunks
        self.step = self.chunk_size // 2
        if self.step == 0:
            raise ValueError("Step size menjadi 0, coba kecilkan n_chunks atau tambah panjang input")
    ...
    def call(self, inputs, training=None):
        ...
        if self.pad_start > 0 or self.pad_end > 0:
            paddings = [[0, 0], [self.pad_start, self.pad_end], [0, 0]]
            padded_inputs = tf.pad(inputs, paddings, "CONSTANT")
        else:
            padded_inputs = inputs
            
        output_buffer = tf.zeros((batch_size, self.padded_seq_len, self.out_filters), dtype=inputs.dtype)
        weight_buffer = tf.zeros((batch_size, self.padded_seq_len, 1), dtype=inputs.dtype)

        starts = list(range(0, self.padded_seq_len - self.chunk_size + 1, self.step))

        for i, start in enumerate(starts):
            end = start + self.chunk_size
            chunk = padded_inputs[:, start:end, :]
            out_chunk = self.global_tcn(chunk, training=training)

            paddings_before = [[0, 0], [start, self.padded_seq_len - end], [0, 0]]
        ...
        output_padded = output_buffer / weight_buffer

        output_final = output_padded[:, self.pad_start : self.pad_start + self.original_seq_len, :]
        return output_final
